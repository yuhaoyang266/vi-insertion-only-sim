...
- Do not claim stage3 is scientifically “truer” than schema2; declare it as the current manuscript-canonical source.
...
- Create: `tests/test_canonical_manifest.py`
- Modify: `README.md:19`
...
**Important Rule:**
- Do not copy stage3 JSON to a new “canonical” filename unless it is an explicitly versioned derived snapshot. The safer default is to keep the original raw artifact file and let the manifest assign the canonical role.

- [ ] **Step 1: Write the failing manifest test**

  Create `tests/test_canonical_manifest.py` asserting that `artifacts/main_benchmark/main_benchmark_manifest.json` exists, has the required top-level roles, and uses repo-relative paths.
...
```bash
python -m pytest -q tests/test_canonical_manifest.py
```
...
- [ ] **Step 1: Write the failing provenance scan**
  ...
  Create `tests/test_artifact_provenance.py` to scan paper-facing paths for forbidden local paths.
...
```bash
python -m pytest -q tests/test_artifact_provenance.py
```
...
- Create: `tests/test_exporter_defaults.py`
...
- [ ] **Step 1: Write default-source tests**
  ...
  Add `tests/test_exporter_defaults.py` asserting Figure 2 and benchmark-table exporters default to the manifest canonical source.
...
```bash
python -m pytest -q tests/test_exporter_defaults.py
...
- Create: `tests/test_paper_table_sync.py`
...
python -m pytest -q tests/test_paper_table_sync.py
```
...
- Modify: `tests/test_three_dof_evidence_matrix.py`
- Modify: `tests/test_sprint2_paper_sync.py:15`
...
- [ ] **Step 1: Fix the reverse test**
  ...
  Rewrite `tests/test_sprint2_paper_sync.py` so it no longer requires schema2 and rejects stage3.
...
python -m pytest -q tests/test_three_dof_evidence_matrix.py tests/test_sprint2_paper_sync.py
```
...
- Create: `tests/test_build_paper_assets.py`
...
python scripts/export/build_paper_assets.py --check
python scripts/export/build_paper_assets.py
python -m pytest -q tests/test_build_paper_assets.py
```
...
- Create: `tests/test_docs_claim_source_sync.py`
...
- Paper text:
  - Replace “schema2 benchmark JSON passed to the exporter” with canonical main benchmark manifest/source hash.
- Figure manifest:
...
- [ ] **Step 1: Write docs sync test**
  ...
  Add `tests/test_docs_claim_source_sync.py` to catch schema2 being described as the main benchmark.

- [ ] **Step 2: Update README top matter**

  Replace the schema2 “tracked frozen main benchmark” wording.
...
```bash
python -m pytest -q tests/test_docs_claim_source_sync.py
```
...
- Modify: `README.md:66`
- Modify: `docs/submission_package_checklist.md:63`
...
- Modify: `tests/test_submission_bundle.py:109`
- Modify: `docs/submission_package_checklist.md:95`
- Modify: `README.md`
...
```bash
python -m pytest -q tests/test_import_boundaries.py
...
```bash
python -m pytest -q tests/test_canonical_manifest.py tests/test_artifact_provenance.py tests/test_paper_table_sync.py tests/test_import_boundaries.py tests/reviewer
python scripts/export/build_paper_assets.py --check
```
...
```bash
python -m pytest -q tests/test_paper_tables.py tests/test_paper_figures.py
python scripts/export/build_paper_assets.py --check
```
...
- Create: `tests/test_paper_claim_boundaries.py`
...
```bash
python -m pytest -q tests/test_paper_claim_boundaries.py
```
