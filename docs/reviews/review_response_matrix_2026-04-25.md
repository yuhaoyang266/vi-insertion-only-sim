...
| SCI is post-hoc and bin-dependent | SCI diagnostic | Major | Bin sensitivity + alternative support metrics | Add fine/default/coarse SCI, state-only/action-only SCI, nearest-demo distance, Jaccard/MMD if stable | SCI remains correlated with contact entry/success across bin settings and is not dominated by trivial distance metrics; otherwise SCI is downgraded | Main summary + appendix full grid | We audited SCI against a 3x3 state/action bin grid and continuous alternatives rather than relying on one discretization. Current artifact populates the predictive schema and synthetic rank-stability checks, but raw-trace SCI values remain weak/zero in the P0 ablation artifacts. Manuscript wording should keep SCI as a bounded diagnostic rather than proof, and P1 should use raw demo/rollout traces or formally downgrade SCI to exploratory. | P0.4 tooling/schema complete; real raw-trace association pending P1 or downgrade |
...
- Interpretation: tooling, schema, and synthetic rank-stability checks are in place, but available P0 artifacts do not contain raw demo/rollout traces and their aggregated SCI values remain weak/zero. Keep SCI as a benchmark-local diagnostic and avoid proof/generalization language until a P1 raw-trace association audit is run; otherwise demote SCI to exploratory appendix diagnostic.
