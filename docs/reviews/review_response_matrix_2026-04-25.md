...
## Sprint B Gate B1 Readout

- B.1 motion-matched main protocol: `artifacts/main_benchmark/three_dof_motion_matched_main_20260429.json`; readout `fi_motion_vi_k` 1.000, `vi_full` 0.800, fixed-K controls 0.000.
- B.2 success-matched mechanics: `outputs/revision/three_dof_impedance_mechanics_20260429.json`; Figure 3 now uses equal 75 successful high-friction traces per method.
- B.3 SCI decision: formal downgrade to exploratory benchmark-local diagnostic because the target artifacts do not persist raw demo/rollout state-action traces.
- B.4 canonical stage4 benchmark: `artifacts/main_benchmark/three_dof_benchmark_paper9suite_full5profile_bc32x32_stage4_20260429.json`; stage4 replaces the prior near-ceiling equality story with widened learned-row gaps.
- Manuscript status: Section 3 now uses stage4 main-table numbers, motion-matched decoupling, success-matched mechanics, and SCI diagnostic-boundary prose.
