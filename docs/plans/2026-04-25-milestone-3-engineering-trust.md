...
**Status:** Complete. Task 10, Task 11, and Task 13 are closed; local PDF/TeX deployment remains outside the Milestone 3 gate.
...
- `tests/core/test_import_boundaries.py` already exists and checks that `import vi_full` does not load `stable_baselines3` or `mujoco`, and does not emit the Gym deprecation warning.
- `.github/` does not exist yet.
- `tests/paper/test_paper_claim_boundaries.py` does not exist yet.
...
- `tests/core/test_import_boundaries.py` only if coverage needs a small assertion.
- `README.md` to document the import contract.

**Checklist:**

- [x] Run the existing boundary test:
  `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 python -m pytest -q tests/core/test_import_boundaries.py`
- [x] Audit `from vi_full import ...` call sites. Only `__version__` should be allowed from top level; runtime APIs should use explicit submodules.
- [x] Confirm `python -c "import vi_full; print(vi_full.__version__)"` works with `PYTHONPATH=src`.
- [x] Add a short README note that `import vi_full` is intentionally lightweight and training/simulator APIs are imported from explicit submodules.
- [x] Close with the import-boundary pytest and direct import command.
...
- [x] Add `tests/paper/test_paper_claim_boundaries.py` with grep-style checks for unsafe phrases and narrow whitelists.
- [x] Scan `paper/main.tex` for broad claims around SG-VI, SCI, DAPG-lite, PPO, generalization, and state-of-the-art wording.
- [x] Patch only the specific overclaiming sentences that fail the test or violate the rules above.
- [x] Run:
  `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 python -m pytest -q tests/paper/test_paper_claim_boundaries.py`
- [x] Do not require local PDF compilation. If visual PDF inspection is needed later, use an external/editor build or an already generated anonymous PDF.
...
- [x] Add `.github/workflows/reviewer-smoke.yml`.
- [x] Add `.github/workflows/paper-assets-check.yml`.
- [x] Keep CI commands as local equivalents in README so failures are reproducible outside GitHub.
- [x] Ensure neither workflow invokes `build_paper_pdf.py`, `pdflatex`, `bibtex`, `latexmk`, or full benchmark training.
- [x] Validate locally with the same no-training commands that do not require TeX/PDF.
