...
def test_report_and_export_capture_per_profile_clearance_drop(tmp_path: Path) -> None:
    ...
    assert artifacts["pdf_path"].suffix == ".pdf"
    assert artifacts["png_path"].suffix == ".png"
...
def test_repo_docs_reference_sprint4_clearance_shift_boundary() -> None:
    ...
    assert "outputs/sprint4_clearance_shift/sprint4_clearance_shift_summary.pdf" in docs
...
def test_phase4_plan_and_protocol_map_stay_in_sync() -> None:
    ...
    assert "- [x] Figure/table pipeline extension" in task_plan
