...
MAIN_BENCHMARK_LATEX_SUITE_ORDER = (
    "ppo_no_bc",
...
MAIN_BENCHMARK_LATEX_DISPLAY_NAMES = {
    "ppo_no_bc": "PPO w/o BC",
    "bc_only_stable_r32_p32": "BC-only",
    "fixed_impedance_rl_stable_r32_p32": "Fixed-impedance RL",
    "repaired_mainline_bc_to_ppo": r"BC$\rightarrow$PPO",
    "dapg_lite_repaired_mainline": "DAPG-lite",
}

MAIN_BENCHMARK_LATEX_HEADER = (
...
def _build_suite_row(
    *,
    ...
    row = {
        ...
        "class_success_statistics": _class_success_statistics(eval_results),
...
def _class_success_statistics(eval_results: dict[str, Any]) -> dict[str, dict[str, float]]:
    statistics: dict[str, dict[str, float]] = {}
    for class_spec in PROFILE_EQUIVALENCE_CLASSES:
        class_name = str(class_spec["class_name"])
        samples: list[float] = []
        for profile_name in class_spec["profiles"]:
            profile_payload = eval_results.get(profile_name)
            if profile_payload is None:
                continue
            for seed_payload in profile_payload.get("per_seed", []):
                if "success_rate" in seed_payload:
                    samples.append(float(seed_payload["success_rate"]))
...
def _latex_pm_cell(mean: float, std: float, *, digits: int) -> str:
    return f"${mean:.{digits}f} \\pm {std:.{digits}f}$"


def _latex_metric_cell(row: dict[str, Any], metric_name: str, *, digits: int) -> str:
    if "five_profile_statistics" not in row:
        return f"${row['five_profile_mean'][metric_name]:.{digits}f}$"
    stats = row["five_profile_statistics"][metric_name]
    return _latex_pm_cell(float(stats["mean"]), float(stats["std"]), digits=digits)


def render_3dof_paper_table_latex(export_payload: dict[str, Any]) -> str:
    rows_by_suite = {row["suite_name"]: row for row in export_payload["suite_rows"]}
    lines = [
        MAIN_BENCHMARK_LATEX_HEADER,
        r"\begin{tabular}{lcccccccc}",
        r"  \toprule",
        r"  Method & Baseline & High fric. & Noisy force & Mean success & Jam & Final dist. (\mm) & Peak force (\N) & Contact steps \\",
        r"  \midrule",
    ]
    for suite_name in MAIN_BENCHMARK_LATEX_SUITE_ORDER:
        row = rows_by_suite[suite_name]
        class_cells = []
        for class_name in ("baseline", "high_friction", "noisy_force"):
            class_stats = row["class_success_statistics"][class_name]
            class_cells.append(
                _latex_pm_cell(
                    float(class_stats["mean"]),
                    float(class_stats["std"]),
                    digits=2,
                )
            )
        cells = [
            MAIN_BENCHMARK_LATEX_DISPLAY_NAMES[suite_name],
            *class_cells,
            _latex_metric_cell(row, "success_rate", digits=3),
            _latex_metric_cell(row, "jam_rate", digits=3),
            _latex_metric_cell(row, "mean_final_distance_mm", digits=3),
            _latex_metric_cell(row, "mean_peak_contact_force_n", digits=3),
            _latex_metric_cell(row, "mean_contact_steps", digits=1),
        ]
