...
Two companion artifacts bound the next steps. The within-A contact-parameter sensitivity artifact,
\code{outputs/revision/contact_parameter_sensitivity_20260501.json}, sweeps all five planned
contact parameters across all five uncertainty profiles, three seeds, and both handcrafted Paper-A
policies. In the five-profile checkpoint, high force-noise sensitivity reduces the variable
impedance success rate on the noisy-force profile, while wall friction remains the largest driver of
peak-force, contact-work, and contact-step deltas. The modern-baseline scaffold,
\code{outputs/revision/modern_baseline_iql_smoke_20260501.json}, selects an IQL/CQL-style offline
baseline path and validates the 14D observation / 5D action offline dataset contract, but does not
yet train an offline RL policy.
...
artifact family rather than an unlanded cross-simulator leaderboard. This framing clarifies the
non-overlap between the two papers: Paper-A studies the support-gated learnability boundary, while
Paper-B studies a force-bounded safety layer. Paper-A does not claim Paper-B's safety guarantee,
and Paper-B is not used here to re-establish the SG-VI or SCI claims. Hardware remains out of
scope, and SCI remains a benchmark-local diagnostic.
