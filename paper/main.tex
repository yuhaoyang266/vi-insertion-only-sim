...
protocol-sensitive and partly non-monotone effects. A separate pure-clearance stress ladder
...
contact: tuned fixed stiffness can nearly saturate success, while the released same-interface
mechanics traces suggest a lower-load variable-impedance contact path without closing the
success-matched mechanics question. The claim is benchmark-local and matched-demonstration bounded,
not a general robotics or teacher-independent algorithm-ranking result.
\end{abstract}
...
per seed and profile. The reviewer-facing claim-control table serves a different role: it ties
nominal-only pure-RL negatives, demo-supported contact-reopening rows, and a mechanics /
fixed-impedance anchor without turning them into a single leaderboard. The cross-family PPO/SAC/TD3
pilot and its confirm report provide a separate
pure-RL negative-evidence layer, Table~\ref{tab:mechanism_matrix} and
Table~\ref{tab:behavioral_similarity} are smaller diagnostic sweeps intended for directional
mechanism analysis, the frozen teacher mini-ablation boundary fixes the next
teacher-support-quality $\times$ demo-budget question, and the pure-clearance stress sweep provides a
...
\code{high\_friction}, where success falls to $0.776 \pm 0.256$. This localized failure mode sets
...
main five-seed benchmark. It uses the existing high-friction trace artifact and keeps an equal
...
not by itself close these external validity questions. Likewise, the pure-clearance stress sweep is
...
nearly saturate success when tuned. This keeps open a localized load-efficiency hypothesis and a
more robust learned interface under the matched-demonstration setup rather than a diffuse claim of
