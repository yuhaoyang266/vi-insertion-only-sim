...
\section{External-Validity Bridge and Checkpoint Evidence}
\label{sec:external_validity_bridge}

The main benchmark remains the simulation-only 3DoF analytical benchmark reported above. The
external-validity bridge is an explicitly separate evidence layer rather than a revision to the
main paper numbers. The bridge is governed by
\code{docs/cross_paper_interface_contract.md}, which pins the 5D Paper-A action interface, the
Paper-B to Paper-A observation projection, shared metrics, the five-profile suite, the offline
dataset schema, and the contract SHA refusal protocol. The Paper-B mirror is currently pinned in
the companion repository, while this paper keeps the learnability claim local to Paper-A.

The current cross-simulator artifact is a contract-level dry-run smoke:
\code{outputs/cross_sim/three_dof_cross_sim_ranking_paper_b_smoke_20260501.json}. It verifies the
contract SHA, Paper-B mirror SHA, action translation, and observation projection, but it does not
run Paper-B physics with learned Paper-A policies. Its rows are therefore marked
\code{not_available} until a real Paper-A policy artifact loader is added. This distinction is
important: the bridge has landed as an auditable interface and artifact path, not as a new
physics-ranking claim.
The artifact metadata separates the Paper-B readiness commit, contract-mirror commit, and actual
checkout commit so that provenance checks are not collapsed into a single ambiguous SHA.

Three companion artifacts bound the fallback path. The within-A contact-parameter sensitivity
artifact, \code{outputs/revision/contact_parameter_sensitivity_20260501.json}, sweeps all five
planned contact parameters across all five uncertainty profiles, three seeds, 20 episodes per seed,
and both handcrafted Paper-A policies. Its summary records 150 aggregate rows, 450 seed summaries,
and 100 paired deltas versus nominal levels. In this checkpoint, high force-noise sensitivity
reduces the variable-impedance success rate on the noisy-force profile, while wall friction remains
the largest driver of peak-force, contact-work, and contact-step deltas. The alternative contact-law
artifact, \code{outputs/revision/alt_contact_model_cross_check_20260501.json}, compares the base
analytical contact law with a within-A soft-wall contact cross-check over the same five profiles and
both handcrafted policies. It is a contact-law stress check inside Paper-A, not a second simulator.
The modern-baseline path now has a concrete offline demonstration artifact,
\code{artifacts/main_benchmark/three_dof_offline_demo_dataset_20260501.json}, and a baseline smoke
artifact, \code{outputs/revision/modern_baseline_iql_smoke_20260501.json}. The dataset records 30
variable-impedance teacher episodes and 1299 samples. The baseline artifact labels the current
method as \code{bc_offline_stub}, keeps IQL/CQL as the target offline-RL direction, and records zero
offline-RL training updates.

This checkpoint therefore defers Paper-B physics ranking until real Paper-A policy artifacts and
completed Paper-B episode records exist. The June fallback path is the within-A sensitivity
artifact family, the contact-law cross-check, and the offline dataset / BC-stub path rather than an
unlanded cross-simulator leaderboard. This framing clarifies the non-overlap between the two
papers: Paper-A studies the support-gated learnability boundary, while Paper-B studies a
force-bounded safety layer. Paper-A does not claim Paper-B's safety guarantee, and Paper-B is not
used here to re-establish the SG-VI or SCI claims. No hardware evidence is included, and SCI
remains a benchmark-local diagnostic.
