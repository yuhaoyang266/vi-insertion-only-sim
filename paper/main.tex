...
Two companion smoke artifacts bound the next steps. The contact-parameter sensitivity smoke,
\code{outputs/revision/contact_parameter_sensitivity_smoke_20260501.json}, runs a small
within-A sweep over \code{contact\_xy\_scale} for handcrafted Paper-A policies and reports no
success-rate overturn in that tiny slice. The modern-baseline scaffold,
...
This section is therefore a checkpoint skeleton for the Tier-2 external-validity path. It clarifies
the non-overlap between the two papers: Paper-A studies the support-gated learnability boundary,
while Paper-B studies a force-bounded safety layer. Paper-A does not claim Paper-B's safety
guarantee, and Paper-B is not used here to re-establish the SG-VI or SCI claims. Hardware remains
out of scope, and SCI remains a benchmark-local diagnostic.
