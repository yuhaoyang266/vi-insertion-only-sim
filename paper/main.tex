...
about $4.35\times 10^{-3}$~J. This diagnostic contrast supports only a bounded interpretation:
fixed impedance is not physically impossible in this reduced task, and the same-interface
hand-crafted variable controller suggests a lower-load, lower-work contact path under high
friction, but a success-matched mechanics split is still needed before making a broader causal
claim. The learned all-trace view remains
...
\begin{figure}[t]
  ...
  hand-crafted fixed-vs-variable pair, where both controllers succeed reliably and the variable
  strategy suggests a lower-load, lower-work contact path under the released trace protocol. The
  hand-crafted fixed anchor is not the tuned fixed-stiffness setting from Appendix~\ref{app:tuned-fixed}; it is a same-interface
